...
// Secure State: No sensitive tokens are exposed to the browser
state([
    'step' => 1,
    'phone' => '',
    'otp' => '',
...
// STEP 1: Normalize Phone, Verify User, and Dispatch OTP
$sendOtp = function ($isResend = false) {
    ...
    // Advanced Normalization: Strip spaces, dashes, brackets, and the leading zero
    $cleanPhone = ltrim(preg_replace('/[^0-9]/', '', $this->phone), '0');
    $throttleKey = 'reset_otp_' . $cleanPhone;

    if (RateLimiter::tooManyAttempts($throttleKey, 3)) {
        $seconds = RateLimiter::availableIn($throttleKey);
        throw ValidationException::withMessages([
            'phone' => "Security lock engaged. Please wait {$seconds} seconds.",
            'otp' => "Security lock engaged. Please wait {$seconds} seconds.",
        ]);
    }
    ...
    if (!$user) {
        ...
        throw ValidationException::withMessages([
            'phone' => 'We could not locate an account matching this mobile number.',
        ]);
    }

    // =======================================================================
    // PRODUCTION SMS API INTEGRATION GOES HERE
    // =======================================================================
    $otpCode = '123456'; // TEMPORARY DEV OTP
    // =======================================================================

    // Securely bind the session to this specific recovery attempt
    session()->put('reset_otp', $otpCode);
    session()->put('reset_user_id', $user->id);
    
    // Log the attempt to prevent SMS spam
    RateLimiter::hit($throttleKey, 60); // Locks for 60 seconds

    $this->step = 2;
    
    // Dispatch browser event to restart the Alpine.js timer if it's a resend
    if ($isResend) {
        $this->dispatch('otp-resent');
    }
};

// STEP 2: Verify the OTP against the Secure Session
$verifyOtp = function () {
    $this->validate([
    ...
    // OTP Verified! Clear the OTP from session but keep the User ID for Step 3
    session()->forget('reset_otp');
    $this->step = 3;
};

// STEP 3: Cryptographically Secure the New Password
$resetPassword = function () {
    $this->validate([
        // Enforce Tier-1 Password Strength
    ...
    if (!$userId) {
        // If the session died, aggressively kick them back to step 1
    ...
    // Obliterate the recovery session
...
<div class="min-h-screen w-full flex flex-col lg:flex-row bg-slate-50 relative overflow-hidden font-sans">
    ...
    <div class="w-full lg:w-[45%] relative z-10 flex flex-col justify-center p-8 lg:p-16 xl:p-24 min-h-[30vh] lg:min-h-screen bg-white border-r border-slate-200 shadow-[4px_0_24px_rgba(0,0,0,0.02)]">
        <div class="max-w-md mx-auto w-full">
            ...
            <p class="text-lg font-bold text-slate-500 leading-relaxed mb-12">
                Regain access to your KoriePay vaults securely. Verify your identity to establish a new cryptographic key.
            </p>
            ...
            <div class="space-y-4 hidden md:block">
                <div class="flex items-start gap-4 p-5 rounded-2xl border border-slate-100 bg-slate-50">
                    ...
                    <div>
                        <p class="text-sm font-black text-slate-900 mb-1">Out-of-Band Verification</p>
                        <p class="text-xs font-bold text-slate-500 leading-relaxed">We use direct telecom SMS routing to ensure only the physical device owner can alter account credentials.</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="w-full lg:w-[55%] flex items-center justify-center p-6 sm:p-12 relative z-10 overflow-y-auto no-scrollbar">
        <div class="w-full max-w-md bg-white border border-slate-200 rounded-[2.5rem] shadow-xl shadow-slate-200/50 p-8 sm:p-12 relative overflow-hidden">
            ...
            <div class="absolute top-0 left-0 h-1.5 bg-gradient-to-r from-[#158987] to-[#29B475] transition-all duration-700 ease-out" style="width: {{ ($step/4)*100 }}%"></div>
            ...
            @if($step === 1)
                <div class="animate-in slide-in-from-right-4 duration-300">
                    <div class="mb-8 text-center sm:text-left">
                        ...
                        <p class="text-xs font-bold text-slate-500 uppercase tracking-widest mt-2">Enter your registered mobile number</p>
                    </div>
                    ...
                    <form wire:submit="sendOtp" class="space-y-5">
                        ...
                        <button type="submit" class="w-full py-5 mt-6 bg-slate-900 text-white rounded-[1.25rem] text-[11px] font-black uppercase tracking-widest hover:bg-slate-800 transition-all active:scale-[0.98] shadow-lg shadow-slate-900/20 flex justify-center items-center gap-3 group">
                            <span wire:loading.remove wire:target="sendOtp">Find My Account</span>
            ...
            @elseif($step === 2)
                <div x-data="{ 
                    ...
                    <div class="mb-8 text-center">
                        ...
                        <p class="text-xs font-bold text-slate-500 mt-2 leading-relaxed">We sent a recovery code to<br><span class="text-slate-900 font-black">*** *** **{{ substr($phone, -2) }}</span></p>
                        <p class="text-[9px] font-black text-orange-500 uppercase tracking-widest mt-3 bg-orange-50 inline-block px-3 py-1 rounded-lg border border-orange-200">Dev Mode: 123456</p>
                    </div>
                    ...
                    <form wire:submit="verifyOtp" class="space-y-6">
                        ...
                        <div class="text-center h-6">
                            <p x-show="timer > 0" class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                                Request new code in <span x-text="timer" class="text-slate-900 font-black"></span>s
                            </p>
                            <button x-show="timer === 0" x-cloak type="button" wire:click="sendOtp(true)" class="text-[10px] font-black text-[#158987] uppercase tracking-widest hover:text-[#29B475] transition-colors">
            ...
            @elseif($step === 4)
                <div class="p-8 text-center space-y-6 animate-in zoom-in-95 duration-500 mt-2">
                    ...
                    <div>
                        ...
                        <p class="text-xs font-bold text-slate-500 uppercase tracking-widest mt-2 px-4 leading-relaxed">Your cryptographic key has been successfully reset.</p>
                    </div>
